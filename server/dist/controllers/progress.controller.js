...
export const getProgressSummary = asyncHandler(async (req, res) => {
    ...
    const [lessons, quizzes, missions, games, achievements, impactRecords,] = await Promise.all([
...
export const upsertLessonProgress = asyncHandler(async (req, res) => {
    ...
    const record = await LessonProgressModel.findOneAndUpdate({ userId, lessonId }, {
...
export const upsertGameProgress = asyncHandler(async (req, res) => {
    const userId = requireUserId(req);
    const { gameId, score, completed, completionTime, isUnlocked, isCompleted, bestScore, lastPlayedAt, progressValue, timesPlayed, completionCount, averageScore, } = req.body;
    ...
    if (typeof isUnlocked === 'boolean')
        updateFields.isUnlocked = isUnlocked;
    if (typeof isCompleted === 'boolean')
        updateFields.isCompleted = isCompleted;
    if (typeof bestScore === 'number')
        updateFields.bestScore = bestScore;
    if (lastPlayedAt)
        updateFields.lastPlayedAt = new Date(lastPlayedAt);
    if (typeof progressValue === 'number')
        updateFields.progressValue = progressValue;
    if (typeof timesPlayed === 'number')
        updateFields.timesPlayed = timesPlayed;
    if (typeof completionCount === 'number')
        updateFields.completionCount = completionCount;
    if (typeof averageScore === 'number')
        updateFields.averageScore = averageScore;
    const record = await GameProgressModel.findOneAndUpdate({ userId, gameId }, updateFields, { new: true, upsert: true, setDefaultsOnInsert: true });
...
export const unlockAchievement = asyncHandler(async (req, res) => {
    ...
    const achievement = await AchievementModel.findOneAndUpdate({ userId, achievementId }, { unlockedAt: new Date() }, { new: true, upsert: true, setDefaultsOnInsert: true });
